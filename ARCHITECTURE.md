...
The `ara2-sys` crate provides low-level FFI bindings to `libaraclient`:

- Generated by `bindgen` from `dvapi.h`
- Uses `libloading` for runtime dynamic library loading
- `ara2::open_library()` tries each name in `ara2::LIBRARY_NAMES` in turn,
  since NXP's packaging has renamed the shared library in every SDK drop so
  far (`libaraclient_aarch64.so`, `libaraclient_x86_64.so`, `libaraclient.so.1`)
