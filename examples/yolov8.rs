...
//! - DMA-backed tensors for zero-copy GPU → NPU preprocessing
...
fn identify_seg_outputs(shapes: &[Vec<usize>]) -> Result<(usize, usize, usize, usize), String> {
    ...
    for (i, s) in shapes.iter().enumerate() {
        ...
        } else if s.len() == 3 && masks.is_none() && protos.map_or(false, |p| shapes[p].get(1) == s.get(1)) {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..n_outputs {
        ...
        // Ultralytics split format. Dividing qn by input_dim normalizes the
        // decoded coordinates to [0, 1].
        ...
        println!(
            "  output[{i}] shape={shape:?} bpp={} signed={} qn={} offset={} (adj_scale={scale:.4})",
    ...
    // ── 6. Warmup ────────────────────────────────────────────────────────
    processor.convert(&src, &mut dst, Rotation::None, Flip::None, letterbox.clone())?;
    model.run()?;
