...
//!
//! Minimal serial example: captures NV12 or YUYV frames from a camera via
//! libcamera, runs YOLOv8 detection + instance segmentation on the ARA-2
//! NPU, and displays results in a native Wayland window.
//!
...
//! ## Usage
//!
//! See `yolov8_live --help` for the full flag reference (generated by clap).
//!
//! ## Requirements
//!
...
/// Live YOLOv8 detection / instance segmentation on the ARA-2 NPU.
///
/// Captures camera frames via libcamera, runs inference on the NPU, and
/// displays results directly on a Wayland compositor via DMA-BUF.
#[derive(Parser)]
struct Args {
    ...
    // Higher default (0.50) than the static example (0.25): live video
    // wants fewer false positives per frame to keep the overlay stable.
    ...
    /// libcamera camera ID. When omitted, the first available camera is used.
    #[arg(long)]
...
/// Camera capture pixel format selectable from the command line.
///
/// This is a command-line mirror of [`PixelFormat`] from `edgefirst-tensor`.
#[derive(Clone, Copy, Debug, ValueEnum)]
enum FormatArg {
    /// Semi-planar YUV 4:2:0 — the embedded-Linux default.
    Nv12,
    /// Packed YUV 4:2:2 — common on USB UVC cameras. Alias: `yuy2`.
    #[value(alias = "yuy2")]
    Yuyv,
}
...
const LIBCAM_NV12: libcamera::pixel_format::PixelFormat =
    libcamera::pixel_format::PixelFormat::new(u32::from_le_bytes(*b"NV12"), 0);
...
const LIBCAM_YUYV: libcamera::pixel_format::PixelFormat =
    libcamera::pixel_format::PixelFormat::new(u32::from_le_bytes(*b"YUYV"), 0);

/// Map a [`FormatArg`] to both the libcamera and HAL pixel format enums.
fn capture_format(fmt: FormatArg) -> (libcamera::pixel_format::PixelFormat, PixelFormat) {
    match fmt {
        FormatArg::Nv12 => (LIBCAM_NV12, PixelFormat::Nv12),
        FormatArg::Yuyv => (LIBCAM_YUYV, PixelFormat::Yuyv),
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (libcam_fmt, hal_fmt) = capture_format(args.format);
        ...
        .expect("No frames from camera");
    {
        ...
        processor.convert(src, &mut model_input, Rotation::None, Flip::None, letterbox)?;
        model.run()?;
